...
def is_six_digit_number(pwd: int) -> bool:
    return 100000 <= pwd <= 999999


def split_pwd(pwd: int) -> List[int]:
    return [int(num) for num in str(pwd)]


def validate_pair(i, nums):
...
def has_two_paired_digits(pwd: int) -> bool:
    nums = split_pwd(pwd)
    ...
    for i in range(1, 6):
        ...
        if previous_num == current_num:
            ...
            are_any_pairs_valid = are_any_pairs_valid or validate_pair(i, nums)
...
def is_valid_pwd(pwd: int) -> bool:
    return is_six_digit_number(pwd) and always_increments_numbers(pwd) and has_two_paired_digits(pwd)
