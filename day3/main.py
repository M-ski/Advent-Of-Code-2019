...
class Segment:
    ...
    def __init__(self, direction: str, length: int, starting_coord: Coordinate):
        ...
        self.length = length
        rel_direction = self.get_relative_direction()
        self.coordinates = [starting_coord]
        for i in range(0, length):
            self.coordinates.append(
                Coordinate(self.coordinates[-1].x + rel_direction.x,
                           self.coordinates[-1].y + rel_direction.y)
            )
        Segment.logger.debug('Created segment wit coordinates: %s', self.coordinates)

    def __str__(self):
        return "({}, {})".format(self.direction, self.length)

    def get_relative_direction(self) -> Coordinate:
...
class Line:
    ...
    def __init__(self, paths: str):
        ...
        for path in paths.split(","):
            ...
            [self.relative_pathing.append(coord) for coord in segment.coordinates[1::]]

    def get_intercepts(self, another) -> List[Coordinate]:
        intercepts = []
        for coordinate in self.relative_pathing[1::]:
            Line.logger.debug('Looking at coordinate %s', coordinate.__str__())
            if coordinate in another.relative_pathing[1::]:
                intercepts.append(coordinate)
        return intercepts
        # return [coordinate for coordinate in self.relative_pathing[1::] if coordinate in another.relative_pathing[1::]]
...
def main():
    ...
    s2 = Segment(Direction.RIGHT, 5, Coordinate(-1, 2))
    l1 = Line(
    ...
    min_distance_2 = min(map(lambda x: x.x + x.y, [x for x in l1.get_intercepts(l3)]))
